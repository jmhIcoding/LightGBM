...
Application::Application(char *model_filename,char *config_file)
{
...
static std::unique_ptr<Predictor> predictor_online;
void Application::InitPredictOnline()
{
		boosting_.reset(
		Boosting::CreateBoosting("gbdt", this->input_model.c_str()));
			//create predictor
		predictor_online=std::unique_ptr<Predictor>(new Predictor(boosting_.get(), config_.num_iteration_predict, config_.predict_raw_score,
		config_.predict_leaf_index, config_.predict_contrib,
		config_.pred_early_stop, config_.pred_early_stop_freq,
		config_.pred_early_stop_margin));
}
void Application::PredictOnline(std::vector<std::string>& vector_datas, int label_index,std::map <int, std::vector<double>  >& result)
{
	predictor_online->Predict(vector_datas,label_index,result);
}
}  // namespace LightGBM
